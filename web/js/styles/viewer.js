export const mediaFeedViewerStyles = `    .cmf-viewer {
    ...
    .cmf-viewer-body {
      /* Use the viewport in both modes so entering comparison keeps the
         metadata width stable whenever the pane has enough room. */
      --cmf-metadata-width: clamp(220px, 16vw, 300px);
      position: relative;
      display: grid;
    ...
    .cmf-viewer-body[data-prompts="true"] {
      grid-template-columns: minmax(0, 1fr) var(--cmf-metadata-width);
      align-items: stretch;
      place-items: stretch;
    }
    ...
    .cmf-viewer-body[data-prompts="true"][data-metadata-position="left"] {
      grid-template-columns: var(--cmf-metadata-width) minmax(0, 1fr);
    }
    ...
    .cmf-viewer-batch-cell::after {
      ...
      left: 0;
      box-sizing: border-box;
      /* Frame the whole cell, including any space around the media. */
      width: 100%;
      height: 100%;
    ...
    .cmf-viewer-media .cmf-viewer-batch-cell video {
      width: 100%;
      height: 100%;
      /* Fit the media element to its picture, including when scaled up. */
    ...
    /* Wrapping in a narrow panel must not push later sections off screen.
       Each value area scrolls independently; the panel itself stays fixed. */
    .cmf-resources-section,
    .cmf-metadata-section {
      max-height: 22vh;
    }
    ...
    @media (min-width: 861px) {
      .cmf-prompt-section {
        /* Keep the heading and at least one line of its value accessible. */
        min-height: 68px;
      }
    ...
    .cmf-viewer[data-comparing="true"] .cmf-viewer-pane[data-metadata-space="true"] {
      grid-template-columns: min(33.333%, var(--cmf-metadata-width)) minmax(0, 1fr);
      gap: 8px;
    }

    .cmf-viewer[data-comparing="true"] .cmf-viewer-pane[data-prompts="true"] .cmf-viewer-media-stage,
    .cmf-viewer[data-comparing="true"] .cmf-viewer-pane[data-metadata-space="true"] .cmf-viewer-media-stage {
      grid-column: 2;
      grid-row: 1;
    }
    ...
    .cmf-viewer[data-comparing="true"] .cmf-viewer-body[data-metadata-position="right"] .cmf-viewer-pane[data-metadata-space="true"] {
      grid-template-columns: minmax(0, 1fr) min(33.333%, var(--cmf-metadata-width));
    }
    ...
    .cmf-viewer[data-comparing="true"] .cmf-viewer-body[data-metadata-position="right"] .cmf-viewer-pane[data-metadata-space="true"] .cmf-viewer-media-stage {
      grid-column: 1;
    }
